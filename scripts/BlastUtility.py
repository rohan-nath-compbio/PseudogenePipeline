...
import FileUtility,FastaManager,sys,os,ParseBlast,time,string,math,tempfile,shutil,subprocess
...
class blast_util:
    ...
    def batch_sw(self,program,pairs,fasta1,fasta2,bdir,outflag=0,wdir="",ev=""):
        ...
        created_wdir = 0
        if wdir == "":
            wdir = tempfile.mkdtemp(prefix="tmp_")
            created_wdir = 1
        ...
        for i in gpairs:
            ...
            tmp_fa1 = os.path.join(wdir, "TMP.FA1")
            tmp_fa2 = os.path.join(wdir, "TMP.FA2")
            tmp_out = os.path.join(wdir, "TMP.OUT")
            ...
            with open(tmp_out, "w") as tmp_handle:
                subprocess.run(
                    [os.path.join(bdir, program)] + flags.split() + [tmp_fa1, tmp_fa2],
                    stdout=tmp_handle,
                    check=True,
                )
            ...
            results = self.parse_tfasty(tmp_out,1)
            if outflag:
                with open(tmp_out, "r") as src, open("%s.sw.raw" % pairs, "a") as dst:
            ...
            for tmp_path in [tmp_fa1, tmp_fa2, tmp_out]:
                if os.path.exists(tmp_path):
                    os.remove(tmp_path)
        
        if created_wdir:
            shutil.rmtree(wdir[:-1], ignore_errors=True)
        print("Done!")
