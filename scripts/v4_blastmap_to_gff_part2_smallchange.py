...
for gene in dict1:
    for chrn in dict1[gene]:
        if chrn not in chrom_intervals:
            chrom_intervals[chrn]=[]
        for lb in dict1[gene][chrn]:
            rb=dict1[gene][chrn][lb]
            chrom_intervals[chrn].append((min(lb,rb), max(lb,rb), gene, chrn, lb, rb))
for chrn in chrom_intervals:
    chrom_intervals[chrn].sort()
############################################
...
while line2:  #the file should be in this format
    ...
    if true==1:
        if ch=='y':
            scan_min=min(start,end)-dist
            scan_max=max(start,end)+dist
        else:
            scan_min=min(start,end)
            scan_max=max(start,end)
        for gene_min, gene_max, gene, chrn, lb, rb in chrom_intervals.get(chr1, []):
            if gene_max < scan_min:
                continue
            if gene_min > scan_max:
                break
            #if abs(lb-start)<=100000 or abs(lb-end)<=100000:  #this would reduce the load on the system
            #25000 is the maximum gene length
